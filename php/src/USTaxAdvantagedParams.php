...
final class Engine
{
    ...
    private static function traits(string $type): array
    {
        ...
        // exceeding a limit on elective deferrals, and the IRC 457(b)(3)
        // last-three-years catch-up raises "the ceiling set forth in paragraph (2)",
        // while IRC 402A(e)(3)(A) forbids any contribution that would push the account
    ...
    private static function baseDeferralLimitForAccount(array $context, array $account, array $traits): ?float
    {
        if (!empty($traits['isPlesa'])) {
            ...
            // deferral is not among the elective deferrals IRC 402(g)(3) enumerates,
            // so an IRC 402A(f)(1)(C) account runs against the IRC 457(e)(15)
            // applicable dollar amount that IRC 457(b)(2)(A) imposes rather than
            // IRC 402(g)(1). The two figures happen to be equal for every year a
            // pension-linked emergency savings account has been available, so this
            // branch is not observable as a dollar difference — only as which pool
            // the contribution is drawn from.
            $statutory = ($traits['family'] ?? null) === 'section457'
    ...
    private static function allocateSection457(array &$context, array $account, array $traits): array
    {
        ...
        $statutoryOrPlesaBase = !empty($traits['isPlesa'])
            ? (self::baseDeferralLimitForAccount($context, $account, $traits) ?? (float) $statutoryBase)
            : (float) $statutoryBase;
        ...
        if (!empty($traits['isPlesa'])) {
            $plesaCap = $context['parameters']['pensionLinkedEmergencySavingsBalanceCap402A'];
            $plesaRoom = self::pensionLinkedEmergencySavingsRoom($context, $account);
            $plesaBalance = $account['planRules']['pensionLinkedEmergencySavingsParticipantContributionBalance'] ?? null;
            if ($plesaCap !== null && $plesaRoom !== null) {
                $suppliedPlesaBalance = self::money($plesaBalance, "accounts.{$account['id']}");
                $diagnostics[] = self::diagnostic(
                    'PENSION_LINKED_EMERGENCY_SAVINGS_BALANCE_CAP_APPLIED',
                    DiagnosticSeverity::INFO,
                    '$' . self::localeNumber((float) $plesaCap)
                        . ' is the IRC 402A(e)(3)(A)(i) ceiling on the portion of the account balance attributable to participant contributions; $'
                        . self::localeNumber($suppliedPlesaBalance)
                        . ' was supplied as already attributable to them, leaving $'
                        . self::localeNumber($plesaRoom)
                        . '. Contributions are Roth by IRC 402A(e)(1)(A)(i) and count against IRC 457(e)(15); IRC 415(c) does not reach an IRC 457(b) plan, and no age-based or last-three-years catch-up is available.'
                        . ' Eligibility under IRC 402A(e)(2), automatic enrollment under IRC 402A(e)(4), the withdrawal right under IRC 402A(e)(7),'
                        . " and the IRC 402A(e)(6)(A) rule directing matching contributions to the participant's other account are not modeled.",
                    "accounts.{$account['id']}",
                    'IRC 402A(e)(3)(A)(i)',
        ...
        // other hosts.
        $specialInput = !empty($traits['isPlesa'])
            ? null
            : ($account['planRules']['section457SpecialCatchUp'] ?? null);
        ...
        return [
            ...
            'statutoryMaximum' => !empty($traits['isPlesa'])
                ? self::roundMoney(self::minMoney(
                    $statutoryOrPlesaBase,
                    $includibleCompensation * (float) $compensationFraction,
                ))
                : self::roundMoney($accountBaseLimit + max($ageLimit, $specialStatutoryExtra)),
            'annualComponents' => $annual,
            'additionalComponents' => $additional,
            'planTermDependentCapacity' => 0.0,
            'sharedLimits' => $sharedLimits,
