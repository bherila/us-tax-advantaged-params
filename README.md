...
Adoption parameters have their own range, **1997 through 2026**, because the Small Business Job
Protection Act of 1996 added the §23 credit and the §137 exclusion for taxable years beginning
after December 31, 1996. See [Adoption parameters](#adoption-parameters).
...
- both of the plan's ceilings bind its records **between them**, not one at a time
  ...
  Existing amounts are measured the same way, so records each inside the ceiling
  alone still raise `SECTION_457_EXISTING_DEFERRALS_EXCEED_PLAN_CEILING` together;
- the plan's **§457(e)(5) includible compensation** is one salary, shared. A dollar
...
`adoptionParametersForYear(taxYear)` returns the §23 adoption credit and §137 adoption assistance exclusion figures for a year. No calculation applies them.

Both provisions carry a `dollarLimit` per adoption, a `specialNeedsDollarLimit`, a flat special-needs amount, and a modified adjusted gross income `phaseout` as `[start, end]`. The credit also carries `codeSection`, `refundability` and `refundablePortionLimit`. The history they record:
...
| 2025–2026 | $17,280, $17,670 | $259,190, $265,080 | up to $5,000, then $5,120, refundable under §23(a)(4) |

The $40,000 phase-out width has never been indexed, and the data validator enforces it for every year.
